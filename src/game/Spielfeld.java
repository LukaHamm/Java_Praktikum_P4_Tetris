...
public class Spielfeld {
			...
			for (int j = 0; j < 10; j++) {
				...
				for (int k = reihen_zum_loeschen.get(i) - 1; k >= 0; k--) {
					...
					if (spielfeldBlockObereReihe != null) {
						spielfeldBlockObereReihe.setLocation(spielfeldBlockObereReihe.getX(),
								spielfeldBlockObereReihe.getY() + Block.BLOCK_SIZE);
					}
	...
	public boolean beruehrt_boden(Form form, int schritt) {
		for (Block block : form.getBlockListe()) {
			if (block != null) {
				...
				for (Block[] spielfeld_reihe : this.spielfeld_blockliste) {
					for (Block spielfeld_block : spielfeld_reihe) {
						if (spielfeld_block != null) {
							if (block.getY() + Block.BLOCK_SIZE + schritt > spielfeld_block.getY()
									&& block.getX() == spielfeld_block.getX()
									&& (block.getY() + schritt - spielfeld_block.getY()) < Block.BLOCK_SIZE) {
								return true;
							}
						}
					}
		...
		for (Block block : form.getBlockListe()) {
			if (block != null) {
				...
				for (Block[] spielfeld_reihe : this.spielfeld_blockliste) {
					for (Block spielfeld_block : spielfeld_reihe) {
						if (spielfeld_block != null) {
							if (block.getX() + schritt == spielfeld_block.getX()
									&& block.getY() + Block.BLOCK_SIZE > spielfeld_block.getY()
									&& (block.getY() - spielfeld_block.getY()) < Block.BLOCK_SIZE) {
								return true;
							}
						}
					}
